...
// Account ID
const val ACC_ID_URI    = "sip:09394241471@etraab"

// Peer to call
const val CALL_DST_URI  = "sip:09394241471@79.175.170.164"
...
const val SIP_LISTENING_PORT = 7079
/* Constants */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        buttonStart.setOnClickListener {
            ...
            try {
                ...
                sipTpConfig.port=7079
                g.ep.transportCreate(pjsip_transport_type_e.PJSIP_TRANSPORT_UDP,
                        sipTpConfig)
                val transportConfig= TransportConfig()
                transportConfig.port=7079

                val accCfg = AccountConfig()
                accCfg.idUri="sip:example@tenent"
                accCfg.regConfig.registrarUri="sip:sipIp:SipPort"
                accCfg.sipConfig.authCreds= AuthCredInfoVector(1,AuthCredInfo("Digest","tenant","username",0,"password"))
                //accCfg.idUri = ACC_ID_URI
                accCfg.videoConfig.autoShowIncoming = true
                accCfg.videoConfig.autoTransmitOutgoing = true
                accCfg.regConfig.headers= SipHeaderVector()
        ...
        buttonCall.setOnClickListener {
            ...
            if (g.call == null) {
                try {
                    ...
                    g.ep.transportCreate(1,cfg)

                    /* Make call (to itself) */
                    val call = MyCall(g.acc, -1)
                    val prm = CallOpParam(false);
                    call.makeCall("sip:number@Ip:Port",prm)
                    g.call = call
                } catch (e: Exception) {
                    println(e)
                }
